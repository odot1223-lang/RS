...
class Executor:
    ...
    def state_init(self, change_state_function):
        ...
        # Webots GPS can report NaN briefly after enabling the device.
        self.sequencer.complex_event(self.robot.has_valid_position)
    ...
    def state_explore(self, change_state_function):
        ...
        if self.stuck_detector.is_stuck():
            print(f"[탐색:executor.state_explore] 끼임 감지 → 탈출 동작 전환")
    ...
    def calibrate_position_offsets(self):
        ...
        tile_size = self.mapper.quarter_tile_size * 2
        position = self.robot.raw_position
        self.robot.position_offsets = Position2D(
            round(position.x / tile_size) * tile_size - position.x,
            round(position.y / tile_size) * tile_size - position.y)
        
    def seq_calibrate_robot_rotation(self):
        """Use one long, straight GPS sample to initialise gyro heading."""
        if self.sequencer.simple_event():
            self.robot.auto_decide_orientation_sensor = False
            self.robot.orientation_sensor = self.robot.GPS
            self.robot.gps.reset_orientation_reference()

        # The supervisor faces the robot toward an open adjacent tile, so this
        # longer forward sample is safe and large enough to dominate GPS noise.
        self.seq_move_wheels(1, 1)
        self.seq_delay_seconds(0.65)

        if self.sequencer.simple_event():
            self.robot.orientation_sensor = self.robot.GYROSCOPE
            self.robot.gps.reset_orientation_reference()

        # Return close to the registered start position before mapping begins.
        self.seq_move_wheels(-1, -1)
        self.seq_delay_seconds(0.65)
        self.seq_move_wheels(0, 0)

    def agent_changed(self):
        """사용 중인 서브 에이전트(follow_walls 등)가 바뀌었는지 체크"""
